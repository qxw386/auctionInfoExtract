...
def grabtextfromImages(imageDiagnose, dateUploaded, isDebug, ctrlDist):
    ...
    for j in range(0, len(d)):
        if d.loc[j, 'distance'] < int(ctrlDist) and ( d.loc[j, 'text'] != 'No.' or int(predictionTextphase) != 1 ):
    ...
    for paragraphIndex in range(d['paragraph'].max()):
        if paragraphIndex == 0:
            ...
            leftDetailData = d[(d['top'] > onlyDetaildataBottome)  & (d['left'] < 300) & (d['paragraph']==paragraphIndex+1)]
    ...
    rightDetailData= d[(d['top'] > onlyDetaildataBottome)  & (d['left'] > 300)]
    if isDebug.lower() == 'true':
...
if __name__ == '__main__':
    ...
    if FileBassedParallelizing.lower() == 'true':
        ...
        print("Proceeding..............................")
        for dateUploaded,fileCSV,isDebug in images2process:
            result = grabtextfromImages(InputPath + dateUploaded + "/" + fileCSV,dateUploaded, isDebug)
            auctionlist.append(result)
